...
export const Home = ({ navigation }) => {
  ...
  const handleView = React.useRef((viewableItems) => {
    ...
    let newState =
      data.item.views &&
      data.item.views
        .filter((view) => view.viewerId === storedCredentials)
        .map((obj) => {
          return obj;
        });
  ...
  const viewConfigRef = React.useRef({
    ...
    minimumViewTimez: 5000,
  });

  useEffect(() => {
    let isLoaded = true;
    if (isLoaded) {
      getCampaigns(setAdsTemp, setUsersInfo, setLikesTemp, setViews);
  ...
  useEffect(() => {
    ...
    if (isLoaded) {
      if ((adsTemp && likesTemp, views)) {
        getAll(adsTemp, views, likesTemp, usersInfo, setAds, dispatch);
      }
    }
    return () => {
      // cancel the subscription
  ...
  const renderAds = ({ item, index }) => {
    ...
    let clildLike;
    clildLike = item.likes
      .filter(
        (obj) => obj.campaignId === item.id && obj.likerId === storedCredentials
      )
      .map((element) => {
        return element;
      });
  ...
  return (
    <>
      <StatusBar barStyle={statusContent} backgroundColor={statusBg} />
